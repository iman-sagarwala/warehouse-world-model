"""30-seed check: does a CONGESTION-AWARE FINISH (traffic delay added before the makeable/doomed
deadline check) help? Compares the champion vs a version that adds FINISH_CONG_K x route-congestion
to the finish. Probe showed no stable K (inert at low, destabilizing at high); K=2.0 = 'bites'.

  rush              - baseline
  parta_congestion  - champion (congestion route/reroute; EMPTY-WORLD finish for the deadline check)
  parta_cong_finish - + congestion-aware finish (K=2.0) feeding the deadline check

Run: python scripts/verify_finish.py     (seeds via SEEDS env, default 30)
"""
...
from congestion_policies import PartACongestionController, PartACongestionFinishController

ENV = "wwm_sim-large-8agvs-4pickers-globalobs-v1"
N_SEEDS = int(os.environ.get("SEEDS", "30"))
SEEDS = list(range(N_SEEDS))
OUT = "results/finish_bakeoff.csv"
...
def make(name, env):
    ...
    if name == "parta_congestion":
        return PartACongestionController(env)
    if name == "parta_cong_finish":
...
POLICIES = ["rush", "parta_congestion", "parta_cong_finish"]
...
def dump(rows):
    ...
    with open(OUT, "w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow([f"# congestion-finish check (K={FINISH_K}), {len(rows)} seeds done"])
...
def main():
    ...
    def mean(p, k):
        return round(sum(r[1][p][k] for r in rows) / len(rows), 1)
    print(f"\n--- MEAN over {len(SEEDS)} seeds (K={FINISH_K}) ---")
