"""30-seed test WITH disturbances: does DISTURBANCE-AWARE ADHERENCE (drop the committed route when a
disturbance blocks it, take the shortest path around) beat the rigid champion? Also vs parta_head
(which edged the champion on value under disturbances).

  champ_rigid    - parta_congestion, adherence fights to rejoin a disturbed route (current)
  champ_release  - parta_congestion, disturb_drop_adherence=True (release on disturbance)
  parta_head     - reference (no committed routes -> reroutes freely)

Run: python scripts/verify_disturb_adhere.py   (seeds via SEEDS env, default 30)
"""
...
DIST_RATE = 0.1
OUT = "results/disturb_adhere_bakeoff.csv"
DL = load_deadlines("data/deadlines_example.txt")
VL = load_values("data/values_example.txt")

POLICIES = ["champ_rigid", "champ_release", "parta_head"]
...
def dump(rows):
    ...
    with open(OUT, "w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow([f"# disturbance-aware adherence check (rate={DIST_RATE}), {len(rows)} seeds"])
