"""Short paired comparison of the congestion variants (15 seeds).

  rush              - baseline
  parta_congestion  - current champion (Part A finish + congestion route/reroute)
  parta_cong_task   - + congestion-aware TASK choice
  rush_cong_full    - RUSH's window + full congestion (task + route + soft reroute)  <- the ask

Paired: seed N identical for all. CSV rewritten each seed. Run: python scripts/verify_variants.py
"""
...
from congestion_policies import (PartACongestionController, PartACongestionTaskController,
                                  RushCongestionFullController)

ENV = "wwm_sim-large-8agvs-4pickers-globalobs-v1"
N_SEEDS = int(os.environ.get("SEEDS", "15"))
SEEDS = list(range(N_SEEDS))
OUT = "results/variants_bakeoff.csv"
...
FACTORY = {
    ...
    "parta_cong_task": PartACongestionTaskController,
    "rush_cong_full": RushCongestionFullController,
}
POLICIES = ["rush", "parta_congestion", "parta_cong_task", "rush_cong_full"]
...
def dump(rows):
    ...
    with open(OUT, "w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow([f"# variants bake-off, {len(rows)} seeds done"])
