"""10-seed check on a MIX of calm + chaotic seeds: does a PROBABILISTIC forecast (spread the
finishing-robot prediction over top-K likely tasks) beat the champion's concentrated top-1 forecast?

  rush              - baseline
  parta_congestion  - champion (top-1 concentrated forecast)
  parta_cong_prob   - forecast spread over top-K likely next tasks (rank-weighted)

Seeds mix calm (0,5,9,20) with chaotic/storm-prone (3,7,15,17,22,24).
Run: python scripts/verify_prob.py
"""
...
from congestion_policies import PartACongestionController, PartACongestionProbController

ENV = "wwm_sim-large-8agvs-4pickers-globalobs-v1"
SEEDS = list(range(int(os.environ.get("SEEDS", "30"))))   # 0-29: all chaotic seeds + calm ones
OUT = "results/prob_bakeoff.csv"
...
FACTORY = {
    ...
    "parta_cong_prob": PartACongestionProbController,
}
POLICIES = ["rush", "parta_congestion", "parta_cong_prob"]
...
def dump(rows):
    ...
    with open(OUT, "w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow([f"# probabilistic forecast check, {len(rows)}/{len(SEEDS)} seeds, mixed calm+chaotic"])
...
def main():
    ...
    def mean(p, k):
        return round(sum(r[1][p][k] for r in rows) / len(rows), 1)
    print(f"\n--- MEAN over {len(SEEDS)} mixed seeds ---")
