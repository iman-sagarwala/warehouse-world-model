"""100-seed head-to-head: which traffic-aware approach wins?

  rush             - scalar baseline (no routes)
  parta_head       - Part A + ORIGINAL (baseline) delay head in the finish estimate
  parta_varhead    - Part A + VARIANCE (congestion-feature) delay head in the finish estimate
  rush_congestion  - Rush selection + CONGESTION MAP route choice + adherence
  parta_congestion - Part A + CONGESTION MAP route choice + adherence  (current champion)

Paired: seed N = identical world for every policy. CSV is rewritten after EACH seed so partial
results survive interruption. Metrics: deliveries, on_time, late, on_time_value, reroutes,
reroutes_per_deliv, stucks.  Run: python scripts/verify_100.py     (seeds via SEEDS env, default 100)
"""
...
from congestion_policies import (PartABaselineHeadController, PartAVarHeadController,
                                  RushCongestionController, PartACongestionController)

ENV = "wwm_sim-large-8agvs-4pickers-globalobs-v1"
N_SEEDS = int(os.environ.get("SEEDS", "100"))
SEEDS = list(range(N_SEEDS))
OUT = "results/bakeoff_100.csv"
...
FACTORY = {
    ...
    "parta_varhead": PartAVarHeadController,
    "rush_congestion": RushCongestionController,
    "parta_congestion": PartACongestionController,
}
POLICIES = ["rush", "parta_head", "parta_varhead", "rush_congestion", "parta_congestion"]
...
def dump(rows):
    ...
    with open(OUT, "w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow([f"# 100-seed bake-off (paired), {len(rows)} seeds done, 500-cap"])
...
def main():
    ...
    for s in SEEDS:
        ...
        rows.append((s, rec))
        dump(rows)      # rewrite after each seed -> partial results survive interruption
        print(f"seed {s}: " + "  ".join(
